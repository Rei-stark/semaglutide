...
def obter_perfil(email):
    resposta = supabase.table('utilizadores').select('*').eq('email', email).execute()
...
# Simulação do Login Google (Para efeitos de demonstração no Streamlit)
email_login = st.text_input("Introduza o seu email (ex: reinaldogalvao@gmail.com ou glaucianaffonseca@gmail.com)")

if email_login:
    perfil = obter_perfil(email_login)
    
    if not perfil:
        st.warning("Perfil não encontrado. Vamos criar a sua conta!")
        with st.form("onboarding"):
            ...
            if st.form_submit_button("Criar Perfil"):
                ...
                supabase.table('utilizadores').insert({
                    'id': novo_id, 'email': email_login, 'nome': nome, 
                    'data_nascimento': str(nascimento), 'sexo': sexo, 'peso_inicial': peso_ini
                }).execute()
                st.success("Perfil criado! Atualize a página.")
    else:
